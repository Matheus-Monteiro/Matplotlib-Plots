...
y_ticks = [i / 4 for i in range(0, 17)]

plt.yticks([y_ticks[i] for i in range(0, len(y_ticks), 2) ])

plt.yticks(fontsize=45)
plt.xticks(fontsize=45)

plt.ylabel('Number of Saturated Links', fontsize=45)
plt.xlabel('Number of Requests', fontsize=45)
plt.title('Average Saturated Links', fontsize=45)
plt.legend(fontsize=48, loc='upper left')


# plt.show()
plt.savefig('sat-bw-ATT.pdf')